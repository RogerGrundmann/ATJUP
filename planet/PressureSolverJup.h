...
public:
    explicit PressureSolverJup(cJupiterModel& model)
        : m(model)
    {}
    ...
    {
        using namespace std;
        cout << endl << endl << endl << "      Jupiter: PressureSolverJup" << endl;
        ...
        for (int i = 0; i < m.im; i++)
            for (int j = 0; j < m.jm; j++)
                for (int k = 0; k < m.km; k++)
                    land[i*m.jm*m.km + j*m.km + k] = is_land(m.SeaMount, i, j, k) ? 1 : 0;
        ...
        for (int i = 1; i < m.im-1; i++) {
            for (int j = 1; j < m.jm-1; j++) {
                ...
                cJupiterModel::CellGeometry geo;
        ...
        // and blows up p_dyn at i=0 near the SeaMount contour; p_dyn is not in bcRadius
        // so that cubic value sits unmodified through the entire RK step and drives
        // radial velocity growth via dpdr at i=1.
        #pragma omp parallel for collapse(2)
        for (int k = 0; k < m.km; k++) {
            for (int j = 0; j < m.jm; j++) {
        ...
        printf(" time measured: %.3f seconds for PressureSolverJup\n", elapsed.count() * 1e-9);
        cout << "      Jupiter: PressureSolverAtm ended" << endl;
    }

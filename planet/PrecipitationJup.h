...
class PrecipitationJup {
public:
    explicit PrecipitationJup(cJupiterModel& model) : m(model) {}
...
inline void PrecipitationJup::run(){
    ...
    printf(" time measured: %.3f seconds for PrecipitationJup\n", elapsed.count() * 1e-9);
    std::cout << "      ATJUP: PrecipitationJup ended" << std::endl;
}

// One condensing species, three-category (rain/snow/graupel).
inline void PrecipitationJup::column(const Species& s){
    ...
    for(int j = 0; j < jm; j++){
        for(int k = 0; k < km; k++){
            ...
            const int i_base = std::max(m.i_topography[j][k], 0);
...
inline void PrecipitationJup::surfaceMap(){
...
inline void PrecipitationJup::sedimentNH4SH(){
    const int im = m.im, jm = m.jm, km = m.km;
    const double t_ref = m.t_ref;

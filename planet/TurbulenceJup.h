...
public:
    ...
    explicit TurbulenceJup(cJupiterModel& model,
...
private:
    ...
    double height_m(int i) const;      // get_layer_height in METRES (ATJUP stores km)
...
inline TurbulenceJup::TurbulenceJup(cJupiterModel& model,
...
inline bool TurbulenceJup::is_land(int i, int j, int k) const {
    return m.SeaMount.x[i][j][k] == 1.0;
}
inline double TurbulenceJup::height_m(int i) const {
    return (double)m.get_layer_height(i) * 1.0e3;    // km -> m
}
inline double TurbulenceJup::L_atm_m() const {
    return m.L_atm * 1.0e3;                          // km -> m
}

inline void TurbulenceJup::print_model_name() const {
    using namespace std;
    switch(turb_model){
...
// -----------------------------------------------------------------------
inline void TurbulenceJup::init(){
    using namespace std;
    cout << endl << "      ATJUP: TurbulenceJup::init" << endl;
    print_model_name();
    ...
    printf(" time measured: %.3f seconds for TurbulenceJup::init\n", elapsed.count() * 1e-9);
    cout << "      ATJUP: TurbulenceJup::init ended" << endl;
}

// -----------------------------------------------------------------------
inline void TurbulenceJup::run(){
    using namespace std;
    cout << endl << "      ATJUP: TurbulenceJup::run" << endl;
    print_model_name();
    auto begin = chrono::high_resolution_clock::now();
    ...
    auto end     = chrono::high_resolution_clock::now();
    auto elapsed = chrono::duration_cast<chrono::nanoseconds>(end - begin);
    printf(" time measured: %.3f seconds for TurbulenceJup::run\n", elapsed.count() * 1e-9);
    cout << "      ATJUP: TurbulenceJup::run ended" << endl;
}
...
inline void TurbulenceJup::apply_wall_bc(){
    const double y1       = height_m(1);                       // first-cell height [m]
    ...
    for(int j = 0; j < m.jm; j++){
        for(int k = 0; k < m.km; k++){
            ...
            // tke wall BC: log-layer equilibrium k = u_tau^2/sqrt(C_mu) (all models).
            m.tke.x[0][j][k] = vs * vs / (std::sqrt(C_nue_l) * m.u_0 * m.u_0);

            // nue wall BC: turbulent viscosity vanishes at the no-slip wall.
            m.nue.x[0][j][k] = 0.0;

            if(turb_model == k_epsilon){
                // Chien (1982) low-Re k-eps: eps_wall = 0 (Dirichlet).
                m.dis.x[0][j][k] = 0.0;
            } else if(turb_model == k_omega){
                // Wilcox 1988: omega_wall = 6 nu/(beta_1 y1^2), beta_1 = 0.075
                const double om_phys = 6.0 * nue_air / (0.075 * y1_safe * y1_safe);
                m.dis.x[0][j][k] = om_phys * nd_omega;
            } else if(turb_model == k_omega_SST){
                // Menter 1994: omega_wall = 60 nu/(beta_1 y1^2), beta_1 = 0.0333
...
inline void TurbulenceJup::init_fields(){
    ...
    for(int j = 1; j < m.jm - 1; j++){
        for(int k = 1; k < m.km - 1; k++){
            ...
            const double y_mount = height_m(m.i_topography[j][k]);   // surface height [m]
...
inline void TurbulenceJup::clamp_nue(){
    const double nue_max = 1000.0 / (m.u_0 * L_atm_m());
    ...
    for(int i = 0; i < m.im; i++){
        for(int j = 0; j < m.jm; j++){
            for(int k = 0; k < m.km; k++){
                const double z_agl = height_m(i) - height_m(m.i_topography[j][k]);
                const double f = clamp3((m.abl_height + taper_dz - z_agl) / taper_dz, 0.0, 1.0);
...
// -----------------------------------------------------------------------
inline void TurbulenceJup::zero_land_cells(){
    #pragma omp parallel for collapse(3) schedule(static)
    for(int i = 0; i < m.im; i++){
        for(int j = 0; j < m.jm; j++){
            for(int k = 0; k < m.km; k++){
...
// -----------------------------------------------------------------------
inline void TurbulenceJup::compute_sources(){
    const double C_nue_l   = C_nue;
    const double dis_min_l = dis_min;
...
// -----------------------------------------------------------------------
inline void TurbulenceJup::compute_k_epsilon(int i, int j, int k, double y_mount,
    double dudr, double dvdr, double dwdr,
    double dudthe, double dvdthe, double dwdthe,
    double dudphi, double dvdphi, double dwdphi,
    double /*Omega*/)
...
// -----------------------------------------------------------------------
inline void TurbulenceJup::compute_k_omega(int i, int j, int k,
    double dtkedr, double ddisdr,
    double dtkedthe, double ddisdthe,
    double dtkedphi, double ddisdphi,
    double Omega_mag, double /*W_unused*/)
...
// -----------------------------------------------------------------------
inline void TurbulenceJup::compute_k_omega_SST(int i, int j, int k,
    double y_mount, double rm, double sinthe,
    double dtkedr, double ddisdr,
    double dtkedthe, double ddisdthe,
    double dtkedphi, double ddisdphi,

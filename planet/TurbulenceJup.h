...
public:
    ...
    static double nue_max_phys(){
        static const double v = [](){ const char* e = getenv("ATJUP_NUE_MAX"); return e ? atof(e) : 1.0e5; }();
        return v;
    }
    ...
    static int abl_taper(){
        static const int v = [](){ const char* e = getenv("ATJUP_ABL_TAPER"); return e ? atoi(e) : 0; }();
        return v;
    }
...
inline void TurbulenceJup::apply_wall_bc(){
    ...
    for(int j = 0; j < m.jm; j++){
        for(int k = 0; k < m.km; k++){
            const int i_w = m.i_topography[j][k];
